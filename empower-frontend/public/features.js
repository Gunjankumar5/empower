document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function triggerSOS(alertType) {
    try {
      ...
      // start live tracking if incident created
      try {
        const incidentId = res?.incident?.id || res?.id;
        if (incidentId && window.LiveTracker) window.LiveTracker.start(incidentId);
      } catch (e) {
        console.warn('LiveTracker start failed:', e && e.message);
      }
